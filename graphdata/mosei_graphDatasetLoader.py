...
class mosei_graphDataset():
    def __init__(self, dataset, name, args) -> None:
        self.args = args
        self._samples = copy.deepcopy(dataset.dataset)
        self.taskname = name

        self.n_max_utterances = max(len(value) for value in dataset.dataset.videoSpeakers.values())
        self.n_max_speakers = 0
        for i in self._samples.keys:
            speaker = self._samples.videoSpeakers.get(i, None)
            if speaker is not None:
                speaker_indices = [s.index(1) +1 for s in speaker if 1 in s]
                self._samples.videoSpeakers[i] = speaker_indices
                unique_speaker_count = len(set(speaker_indices))
                self.n_max_speakers = max(self.n_max_speakers, unique_speaker_count)
        ...
        self.num_samples = self._samples.len
        self.num_batches = 1
        self.batch_size = self.num_samples
        if name == 'train' or name == 'pretrain':
            self.batch_size = args.batch_size
            self.num_batches = math.floor(self.num_samples / args.batch_size)

        self.batch_sizes = [self.batch_size]*(self.num_batches)
        self.samples = list()
        self.setup()
        self.batches = list()
        self.set_batch()
    ...
    def setup(self):
        ...
        for i in self._samples.keys:  # 0부터 1431까지 반복
            data = {
                # Data
                "audio": None,
                "text" : None,
            ...
            audio = self._samples.videoAudio.get(i, None)
            speakers = self._samples.videoSpeakers.get(i, None)
            text = self._samples.videoText.get(i, None)
            visual = self._samples.videoVisual.get(i, None)

            label_tensor = None
            if not self.taskname =='pretrain':
                label = self._samples.videoLabels.get(i, None)
                label_tensor = torch.full((self.n_max_utterances,), -1, dtype= torch.long)
                label_tensor = self._padding(label, label_tensor, dtype=torch.long)


            speaker_tensor = torch.zeros_like(audio[:,0]).to(torch.long)
